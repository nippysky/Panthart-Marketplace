 ...
 * Component
 * ──────────────────────────────────────────────────────────────────────────────*/
export type WithdrawProceedsDialogProps = {
  contract: string;       // collection contract
  collectionName: string; // nice label
  ownerAddress: string;   // visibility gate handled by parent
  className?: string;
};
...
export default function WithdrawProceedsDialog({
  contract,
  collectionName,
  ownerAddress, // (unused here; gating done by parent)
  className,
}: WithdrawProceedsDialogProps) {
        ...
        >
          <div className="flex h-full flex-col">
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 sm:px-6 sm:py-4 space-y-4">
              ...
              {/* ── Last transaction: fully fluid, never distorts layout ───────── */}
              {lastTx && (
                <div className="rounded-lg border bg-muted/10 p-3 sm:p-4 space-y-2">
                  ...
                  {/* The pill that holds the long hash:
                       - wraps anywhere (break-words + overflow-wrap:anywhere)
                       - allows inner horizontal scroll on ultra-narrow devices
                       - never grows outside the modal */}
              ...
              <p className="text-xs text-muted-foreground">
                Only the collection owner can withdraw mint proceeds from this
                contract. Your wallet will be prompted to confirm the transaction
                on the <b>{ELECTRONEUM.name}</b> network.
              </p>
            </div>
